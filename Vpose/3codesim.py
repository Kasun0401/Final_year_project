...
resized_img = img.subsample(2, 2)  # Example: reduce the size by a factor of 2 in both dimensions
...
# Initialize the GSM module (assuming it's connected to the Raspberry Pi UART pins)
ser = serial.Serial('/dev/ttyS0', 9600, timeout=5)  # Use '/dev/ttyS0' for UART0 on Raspberry Pi 4
...
def send_sms(message, recipient):
    # Send SMS using AT commands to the SIM900A module
    ser.write('AT+CMGF=1\r'.encode())  # Set SMS mode to text
    time.sleep(1)
    ser.write(f'AT+CMGS="{recipient}"\r'.encode())  # Replace 'recipient' with the recipient's phone number
    time.sleep(1)
    ser.write(f'{message}\x1A'.encode())  # Message body followed by CTRL+Z
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        ...
        if prev_pred != pred and not message_sent:
           ...
           # Send SMS using SIM900C
            send_sms(f"Gesture Recognized: {pred}", "0718684441")  # Replace '1234567890' with the recipient's phone number
            prev_pred = pred
            message_sent = True
